...
@app.after_request
def add_session_refresh_headers(response):
    """Add headers to help with session persistence across devices"""
    response.headers['Access-Control-Allow-Credentials'] = 'true'
    return response

# Session refresh endpoint to keep session alive
@app.route('/api/session/refresh', methods=['GET', 'OPTIONS'])
def session_refresh():
    """Refresh session and validate remember token"""
    ...
    if 'user_id' in session:
        # Session exists, just refresh it
        session.permanent = True
        return jsonify({
            "status": "ok", 
    ...
    return jsonify({"status": "expired", "session_valid": False}), 401
...
def compare_prices():
    ...
    if len(urls) > 5:
        return jsonify({"error": "Maximum 5 URLs can be compared at once"}), 400
    ...
    for url in urls:
        ...
        try:
            ...
            for candidate_url in fetch_candidates[:2]:
                headers = get_request_headers(candidate_url, site)
                current_response = session_client.get(candidate_url, headers=headers, timeout=15, allow_redirects=True)
                ...
                if current_price is None:
                    candidates = extract_price_candidates(current_response.text)
                    if candidates:
                        current_price = min(candidates)
            ...
            if soup and soup.title and soup.title.get_text():
                title = soup.title.get_text().strip()
                product_name = re.sub(r'\s*[-|]\s*(Amazon|Flipkart|Myntra|Ajio|Meesho|Snapdeal)\s*$', '', title, flags=re.IGNORECASE).strip()
            og_title = soup.find("meta", attrs={"property": "og:title"}) if soup else None
            if og_title and og_title.get("content"):
                product_name = og_title.get("content").strip()
            ...
            if price is not None:
                ...
                results.append({"url": url, "error": "Could not fetch price"})
    ...
    return jsonify({
        "results": results,
        "bestPrice": valid_results[0] if valid_results else None,
